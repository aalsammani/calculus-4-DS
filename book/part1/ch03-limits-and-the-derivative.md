# 2.1 · Limits and the Derivative

This section answers the founding question of differential calculus: *what is the instantaneous rate of change of a function?* Average rates over an interval are elementary — the difference quotient of Section 1.1 computes them. The difficulty is that "instantaneous" seems to demand a rate over an interval of length zero, which would be the meaningless expression $0/0$. The resolution is the idea of a **limit**, and we develop exactly as much limit theory as the derivative requires: enough to be correct, no more.

## 2.1.1 Motivation: from average to instantaneous
...
One famous limit cannot be found by algebra and will be *needed* to differentiate sine in Section 2.3, so we record it now with numerical evidence:
...
Section 1.1 ({prf:ref}`ex-diff-quotient`) already did the algebra: for $h \neq 0$,
...
## 2.1.4 Do it by hand: a complete workflow

Before any shortcut rules exist (they arrive in Section 2.2), here is the full manual procedure on a fresh problem. Find the equation of the tangent line to $f(x) = x^2 - 4x + 1$ at $x = 3$.
...
**Treating the derivative as a fraction of two numbers.** The Leibniz symbol $\frac{df}{dx}$ is a single indivisible notation for a limit, not a ratio $df \div dx$ (although its fraction-like behavior in Sections 2.3 and 3.2 is a deliberate, and provable, design feature of the notation).
...
# --- Verify Example 2.1.3: d/dx [x^2 - 4x + 1] = 2x - 4 ---
f = lambda x: x**2 - 4*x + 1
for x0 in [0.0, 1.0, 3.0]:
    print(numerical_derivative(f, x0), 2*x0 - 4)   # pairs should match

# --- Verify Example 2.1.4: d/dx [1/x] = -1/x^2, at x = 2 ---
print(numerical_derivative(lambda x: 1/x, 2.0), -1/4)

# --- Verify Example 2.1.5: d/dx [sqrt(x)] = 1/(2 sqrt x), at x = 9 ---
...
:class: tip
The derivative is the mathematics of **sensitivity**: $f'(x)$ tells you how much the output moves per unit nudge of the input. "How much does predicted revenue change per dollar of price?" and "how much does the loss change per unit change in this model weight?" are both derivative questions, and the second one — evaluated for millions of weights at once — is what *training* a model means. The gradient of Chapter 6 is this section's derivative grown to many variables.
```

```{admonition} Looking Ahead
:class: seealso
Computing every derivative from the limit definition would be unbearable. Section 2.2 harvests the definition once and for all into a small set of **rules** that reduce differentiation to mechanical algebra. The limit definition remains the meaning; the rules become the method.
```
...
So $\dfrac{d}{dx}x^3 = 3x^2$ — one more data point for the pattern ($x^2 \mapsto 2x$, $x^3 \mapsto 3x^2$) that Section 2.2 will name the power rule.
````

````{admonition} Solution to Exercise 7
:class: dropdown
From 6(b), $f'(x) = 2x + 3$, so the slope at $x = -1$ is $f'(-1) = 2(-1) + 3 = 1$. The point of tangency is $(-1, f(-1)) = (-1, (-1)^2 + 3(-1)) = (-1, -2)$. Point–slope form:

$$
y - (-2) = 1\,(x - (-1)) \quad\Longrightarrow\quad y = x - 1.
$$

**Check:** the line and parabola agree at $x = -1$ (both give $-2$), and the line's slope $1$ matches the derivative there. A tangent line *is* the linearization — a preview of §2.4.1.
````
...
18. Using the definition of the derivative and the limit $\lim_{h\to 0}\frac{\sin h}{h} = 1$ together with $\lim_{h \to 0}\frac{\cos h - 1}{h} = 0$ (which you may assume), prove that $\frac{d}{dx}\sin x = \cos x$. *(Hint: expand $\sin(x+h)$ with the angle-sum identity from Section 1.2.)*
