# 2.4 · Linearization and Newton's Method

This section cashes in the derivative for its two most practical dividends. The first, **linearization**, exploits the defining fact about tangent lines — that they hug their curve — to replace complicated functions by lines, locally, with controllable error. The second, **Newton's method**, iterates that replacement to solve equations that have no algebraic solution, and does so with astonishing speed. Between them these ideas power calculators, numerical libraries, and the optimization routines that train statistical models; they are the first place in the book where calculus stops describing and starts *computing*.
...
Choose a nearby point where the function is easy: $a = 4$, where $f(x) = \sqrt x$ gives $f(4) = 2$ and, by Section 2.1, $f'(x) = \frac{1}{2\sqrt x}$ so $f'(4) = \frac14$. The linearization is
...
The error $f(x) - L(x)$ is governed by the **second** derivative: the tangent line matches $f$ in value and slope at $a$, so what it misses is curvature. The precise statement (proved via Taylor's theorem in Section 4.2) is that for $x$ near $a$,
...
and continuing (by the same arithmetic, done in §2.4.4's code): $x_2 = 0.739113$, $x_3 = 0.739085$, at which point $x_4$ agrees with $x_3$ to all twelve printed digits. The unique real solution of $\cos x = x$ is $x = 0.739085\ldots$ — a number with no closed form, delivered to machine precision in three steps.
```
...
# --- Verify Example 2.4.5: sqrt(2), reproducing the hand table ---
xs = newton(lambda x: x**2 - 2, lambda x: 2*x, x0=2.0)
for n, x in enumerate(xs):
    print(f"x_{n} = {x:.12f}   error = {abs(x - np.sqrt(2)):.3e}")

# --- Verify Example 2.4.6: the root of cos(x) = x ---
xs = newton(lambda x: np.cos(x) - x, lambda x: -np.sin(x) - 1, x0=1.0)
print([f"{x:.6f}" for x in xs])       # 1.0, 0.750364, 0.739113, 0.739085, ...

# --- Verify Example 2.4.2: linear approximation of sqrt(4.2) ---
L = lambda x: 2 + (x - 4)/4
print(L(4.2), np.sqrt(4.2))           # 2.05 vs 2.049390...
```

The first block reproduces the error table of {prf:ref}`ex-newton-sqrt2` digit for digit; the second confirms $0.739085$; the third shows estimate and truth side by side. SymPy supplies the derivative when differentiating by hand would be tedious, closing the loop between this section and the last:
...
8. For $f(x) = \sqrt{1+x}$: find the linearization at $a=0$, use it to estimate $\sqrt{1.06}$, and then use the error bound of §2.4.1 with $M = \max|f''|$ on $[0, 0.06]$ to give a guaranteed bracket for the true value.
...
16. Solve $e^{-x} = x$ to twelve digits, obtaining the derivative via SymPy's `lambdify` as in §2.4.4. Report the root and the number of iterations from $x_0 = 0.5$.
...
18. Reproduce the semilog convergence plot of §2.4.4 for the root of $x^3 - x - 2$ (Exercise 9), and add, for comparison, the error sequence of simple **bisection** on the same problem starting from your bracketing interval. Describe the visual difference between quadratic and linear convergence.
