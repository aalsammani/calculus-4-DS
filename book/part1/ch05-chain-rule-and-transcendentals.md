# 2.3 · The Chain Rule and the Transcendental Functions

Section 2.2's rules handle sums, products, and quotients, but they are silent on the most common structure in applied mathematics: **composition**. Functions in practice are nested — $e^{-x^2}$, $\ln(1 + x^2)$, $\sin(2\pi t)$, $\sqrt{3x+1}$ — and every one of them defeats the toolkit so far. This section supplies the missing rule, the chain rule, and then uses it (together with two special limits) to differentiate the transcendental functions: sine, cosine, and their relatives; the exponential; and the logarithm. By the section's end you will be able to differentiate essentially any function built from the standard parts.
...
## 2.3.2 Derivatives of the trigonometric functions

The chain rule needs raw materials: derivatives of the basic functions to put at the ends of chains. For sine, return to the limit definition, expand with the angle-sum identity of Section 1.2, and separate:
...
Section 2.1 established $\lim_{h\to0}\frac{\sin h}{h} = 1$, and the companion limit $\lim_{h\to0}\frac{\cos h - 1}{h} = 0$ follows from it (multiply by the conjugate $\frac{\cos h + 1}{\cos h + 1}$ and use $\sin^2 h = 1 - \cos^2 h$). Taking $h \to 0$:
...
## 2.3.3 The exponential function

Section 1.2 promised that $e$ is the base whose exponential has slope equal to height. Here is where that comes from. For any base $b > 0$, the difference quotient of $b^x$ factors remarkably:
...
## 2.3.4 The logarithm, and logarithmic differentiation
...
This is a small marvel: the transcendental function $\ln x$ has the perfectly algebraic derivative $1/x$ — the one power of $x$ that the power rule for antiderivatives will miss in Section 3.1, a coincidence that is not a coincidence. For other bases, change of base gives $\log_b x = \frac{\ln x}{\ln b}$, so $\frac{d}{dx}\log_b x = \frac{1}{x\ln b}$. And composing with an inner function:
...
The same $\ln$-both-sides move also settles old business: for any real $n$ and $x>0$, $\ln(x^n) = n\ln x$ gives $\frac{y'}{y} = \frac nx$, so $y' = n\frac{x^n}{x} = nx^{n-1}$ — the power rule for **all real exponents**, as promised in Section 2.2.
...
## 2.3.6 Now do it in Python

SymPy knows every rule in this section; the point of the code is to *check our hand answers* and to see the chain rule as an algorithm.
...
**Interpretation.** The derivative curve crosses zero exactly under the bell's peak, is positive on the rising left flank and negative on the falling right flank, and has its own extremes where the bell is steepest (at $x = \pm 1/\sqrt2$, computable by setting the *second* derivative to zero — try it). Reading paired $f$/$f'$ plots fluently is a skill worth deliberate practice; it is how practitioners debug models and how Section 2.4 will reason about approximation error.

```{admonition} Data Science Connection
:class: tip
**Backpropagation is the chain rule.** A neural network is a long composition — linear map, nonlinearity, linear map, nonlinearity, … , loss — and training requires the derivative of the loss with respect to every parameter. Backpropagation computes it by applying the chain rule layer by layer from the outside in, exactly as in {prf:ref}`ex-chain-triple`, caching each layer's local derivative and multiplying. The derivative $\sigma' = \sigma(1-\sigma)$ of the logistic function and the $u'/u$ form of log-loss gradients are both single lines from this section.
```

```{admonition} Looking Ahead
:class: seealso
Every derivative formula in this section will be read *backwards* in Chapter 3 as an antiderivative formula, and the chain rule read backwards becomes **substitution** (Section 3.2), the most important integration technique. The chain rule also returns in many-variable form in Section 6.3, where it underlies the gradient computations of optimization.
```
...
20. Differentiate $y = \ln\!\bigl(x + \sqrt{x^2+1}\bigr)$ and simplify fully; the answer is remarkably clean. *(This function, the inverse hyperbolic sine, returns in Section 3.4.)*
