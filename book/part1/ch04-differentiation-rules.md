...
Section 2.1 defined the derivative as a limit and computed a handful of examples by hand. Each took several lines of careful algebra. Since differentiation is something applied mathematics does constantly, computing every derivative from the definition would be like doing all arithmetic with tally marks. This section converts the definition, once and for all, into a small toolkit of **rules** — worked out from the limit definition so you know they are theorems rather than conventions — after which differentiating becomes fast, reliable algebra.

## 2.2.1 The power rule and linearity

Section 2.1 found a pattern: $\frac{d}{dx}x^2 = 2x$ and $\frac{d}{dx}x^3 = 3x^2$; the exponent drops down as a coefficient and decreases by one. This pattern holds far more generally.
...
and as $h \to 0$ all trailing terms vanish, leaving $n x^{n-1}$. The rule extends to negative and fractional exponents (Section 2.1's hand computations $\frac{d}{dx}x^{-1} = -x^{-2}$ and $\frac{d}{dx}x^{1/2} = \tfrac12 x^{-1/2}$ are exactly the power rule at $n = -1$ and $n = \tfrac12$), and to all real exponents once logarithmic differentiation is available in Section 2.3.
...
and each differentiation lowers every degree by one, so $p^{(6)} = 0$ identically: a degree-5 polynomial has six nonzero derivatives at most. Second derivatives return in Section 2.4 (error of linearization) and Section 4.2 (Taylor's quadratic term).

## 2.2.5 Common mistakes
...
**Differentiating through a power incorrectly:** $\frac{d}{dx}(x^2+1)^3$ is *not* $3(x^2+1)^2$ — a composition is hiding inside, and it needs the chain rule (Section 2.3). Until then, expand first if you must differentiate such a thing.
...
## 2.2.6 Now do it in Python

SymPy applies every rule of this section exactly, which makes it the ideal answer-checker for hand practice. NumPy's central differences supply an independent numerical check.
...
# --- Verify Example 2.2.3 (product rule) ---
y = (x**2 + 1) * (x**3 - 2*x)
print(sp.expand(sp.diff(y, x)))          # expect 5*x**4 - 3*x**2 - 2

# --- Verify Example 2.2.5 (quotient rule) ---
y = x**2 / (x + 3)
print(sp.simplify(sp.diff(y, x)))        # expect x*(x + 6)/(x + 3)**2

# --- Verify Example 2.2.7 (marginal revenue) ---
p = sp.symbols('p')
R = p * (1200 - 40*p)
Rp = sp.diff(R, p)
...
:class: tip
Modern machine-learning frameworks differentiate enormous functions by **automatic differentiation**: they represent a computation as elementary operations and apply exactly this section's rules (plus the chain rule) mechanically, propagating derivatives through sums, products, and quotients. When a library "computes gradients," it is executing the product rule at scale. The rules you are practicing by hand are the literal instruction set of that machinery.
```

```{admonition} Looking Ahead
:class: seealso
The product and quotient rules assume you can already differentiate the *pieces*; §2.3 supplies the missing machinery — the chain rule — for pieces that are themselves compositions, and extends the rule set to exponentials, logs, and trig. The second derivative introduced here becomes §2.4's curvature information, the classifying tool of §2.5's second-derivative test, §4.2's Taylor coefficients, and ultimately the Hessian of §6.3.
```
