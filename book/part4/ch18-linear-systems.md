# 5.4 · Linear Systems and Gauss–Jordan Elimination
...
— find the input $\vb x$ that the machine $\mathit A$ sends to the output $\vb b$. Fitting a curve through data points, balancing a chemical equation, finding equilibrium prices, inverting a transformation, computing regression coefficients: all are this problem wearing different clothes. This section develops the algorithm that solves it — **Gauss–Jordan elimination** — together with the geometry of what solutions look like (one, none, or infinitely many), and the notion of an inverse matrix, including the crucial practical advice about when *not* to compute one.

## 5.4.1 Two pictures of one system
...
can be read through either lens from Section 5.3. The **row picture**: each equation is a line, and solving means finding where all the lines meet. The **column picture**: by the column reading of $\mathit A\vb x$, the system asks for the *linear combination of the columns that produces $\vb b$*:
...
## 5.4.2 Elimination: the algorithm
...
Elimination's vocabulary — pivots, free variables, rank — deserves three companion words that name what the *columns* are doing.

```{prf:definition} Span, linear independence, basis
:label: def-span-basis
The **span** of vectors $\vb v_1, \ldots, \vb v_k$ is the set of all their linear combinations $c_1\vb v_1 + \cdots + c_k\vb v_k$ — every point they can reach. The vectors are **linearly independent** if the only combination producing $\vb 0$ is the trivial one ($c_1 = \cdots = c_k = 0$); equivalently, no vector in the list is a combination of the others. An independent set that spans a space is a **basis** for it, and every basis of a given space has the same size — the space's **dimension**.
```

Every one of these properties is decided by the machinery already on the table, because $c_1\vb v_1 + \cdots + c_k\vb v_k = \vb 0$ *is* the homogeneous system $\mathit A\vb c = \vb 0$ whose coefficient matrix has the $\vb v_i$ as columns. Independence means that system has only the trivial solution — which elimination reads off instantly: **the columns are independent exactly when every column receives a pivot**, i.e. when the rank equals the number of vectors. A free variable, by contrast, hands you a *nontrivial* combination equal to zero: a dependency, made explicit.

A small case shows the whole method. Are $\vb v_1 = \langle 1,2,0\rangle$, $\vb v_2 = \langle 0,1,1\rangle$, $\vb v_3 = \langle 1,4,2\rangle$ independent? Eliminate on the matrix with these columns:

$$
\begin{bmatrix} 1&0&1\\ 2&1&4\\ 0&1&2 \end{bmatrix}
\;\longrightarrow\;
...
Rank $2$ with three columns: **dependent**, and the pivotless third column even reports the recipe — setting the free variable $c_3 = -1$ gives $c_1 = 1$, $c_2 = 2$, so $\vb v_3 = \vb v_1 + 2\vb v_2$ (check: $\langle 1,2,0\rangle + \langle 0,2,2\rangle = \langle 1,4,2\rangle$ ✓). The two pivot columns $\vb v_1, \vb v_2$ are a basis for the span of all three: the third vector contributed reach the first two already had. This is the general pattern — *the pivot columns of a matrix form a basis for the span of its columns*, so elimination doubles as a redundancy detector.

The data-science reading is direct. A dataset's feature columns that are linearly dependent (a feature recorded in both meters and feet, a total alongside its parts) make the design matrix rank-deficient — precisely the collinearity that renders regression weights non-unique in this section's Data Science Connection below. "How many genuinely different directions of information does my data contain?" is the question *rank* answers, and span, independence, and basis are its grammar.

## 5.4.3 Inverse matrices
...
— swap the diagonal, negate the off-diagonal, divide by $ad - bc$ (a first sighting of the determinant). For the system of §5.4.1, $\mathit A = \begin{bmatrix}1&2\\3&-1\end{bmatrix}$ has $ad - bc = -7$, so
...
# --- Example 5.4.1: exact RREF, watching Gauss-Jordan finish ---
M = sp.Matrix([[1, 1, 1, 6],
               [2, -1, 1, 3],
               [1, 2, 3, 14]])
rref, pivots = M.rref()
...
# --- Example 5.4.2: the singular cases, diagnosed by rank ---
...
:class: dropdown
$\det \mathit A = 4\cdot2 - 7\cdot1 = 1$, so the inverse formula gives

$$
\mathit A^{-1} = \frac{1}{1}\begin{bmatrix} 2 & -7\\ -1 & 4 \end{bmatrix}
= \begin{bmatrix} 2 & -7\\ -1 & 4 \end{bmatrix},
\qquad
\mathit A\mathit A^{-1} = \begin{bmatrix} 8-7 & -28+28\\ 2-2 & -7+8 \end{bmatrix} = \mathit I \ \checkmark
$$

Then $\mathit A\vb x = \langle 1, 0\rangle$ has solution $\vb x = \mathit A^{-1}\langle 1,0\rangle = \langle 2, -1\rangle$, and $\mathit A\vb x = \langle 0,1\rangle$ gives $\vb x = \langle -7, 4\rangle$. As a pair, those two solutions are exactly the **columns of $\mathit A^{-1}$** — which is no accident: solving against the identity's columns is precisely what "finding the inverse" means, and it is how Gauss–Jordan computes inverses in practice.
````
...
15. Why can a linear system never have exactly two solutions? Give the one-line linearity argument from §5.4.1 in your own words, and state what it implies about the *shape* of any solution set (point, line, plane, …).
...
19. Plot the row pictures of Exercise 6(a)–(b): coincident lines versus parallel lines. Then plot §5.4.5's near-singular pair and the solution's jump under a $1\%$ intercept perturbation, annotating both intersection points.
...
22. **Leontief input–output model.** An economy's sectors consume each other's output per unit produced according to $\mathit C = \begin{bmatrix}0.2 & 0.3\\ 0.4 & 0.1\end{bmatrix}$; external demand is $\vb d = \langle 100, 120\rangle$. Total production must satisfy $\vb x = \mathit C\vb x + \vb d$, i.e. $(\mathit I - \mathit C)\vb x = \vb d$. Solve for $\vb x$; then verify numerically that $(\mathit I - \mathit C)^{-1} = \mathit I + \mathit C + \mathit C^2 + \cdots$ by summing 50 terms — a matrix geometric series (Section 4.1's formula, grown up), convergent because $\mathit C$'s entries drain rather than amplify.

23. Decide whether $\langle 1,1,0\rangle$, $\langle 1,0,1\rangle$, $\langle 0,1,1\rangle$ are linearly independent. If they are, explain why they form a basis of $\R^3$, and find the coordinates of $\langle 2,3,5\rangle$ in that basis (solve the appropriate system; the answer is $c_1 = 0$, $c_2 = 2$, $c_3 = 3$ — verify it).
24. Prove that *any* three vectors in $\R^2$ are linearly dependent, using ranks: how many pivots can a $2\times 3$ matrix have, and what does a pivotless column guarantee? Generalize the statement to $k$ vectors in $\R^n$ with $k > n$.

### Cumulative Review

25. *(§5.3)* For the shear $\mathit S = \begin{bmatrix}1&1\\0&1\end{bmatrix}$, guess $\mathit S^{-1}$ from the transformation's meaning (shear back), verify by multiplication, and reconcile with the $2\times2$ formula.
26. *(§2.4)* Newton's method in Section 2.4 divided by $f'(x_n)$. Its many-variable descendant solves the *linear system* $\mathit J\,\Delta\vb x = -\vb f(\vb x_n)$ at each step, where $\mathit J$ collects partial derivatives (Section 6.2). In one sentence each: what plays the role of $f'$, and why does "divide" become "solve"?

## 5.4.7 Summary

$\mathit A\vb x = \vb b$ reads as intersecting rows or as combining columns, and its solution set is always a point, nothing, or an infinite affine family — never anything in between. Gauss–Jordan elimination reaches the answer by three solution-preserving row operations on the augmented matrix: create pivots, zero below (echelon form, then back-substitute) or below *and* above (RREF, read off directly). Contradiction rows flag inconsistency; pivotless columns flag free variables, with solutions written as particular + span of directions; the pivot count — the rank — measures the system's independent information, and the same machinery decides span, linear independence, and basis: columns are independent exactly when each earns a pivot, and the pivot columns form a basis for the columns' span. Square matrices with full rank are invertible ($\mathit A^{-1}\mathit A = \mathit I$; $2\times2$ by the $ad-bc$ formula, general by $[\,\mathit A\mid\mathit I\,]$ reduction; $(\mathit{AB})^{-1} = \mathit B^{-1}\mathit A^{-1}$), but computational practice solves systems by elimination (`np.linalg.solve`), not by inverses, and watches conditioning where columns are nearly dependent. Regression's normal equations, interpolation, and equilibrium models are all this one problem; the determinant, next, is invertibility distilled to a single number.

*Parallel reading:* Strang {cite}`strang_linalg`, Sections 1.2–2.1; Goodfellow et al., §2.3–2.4 {cite}`goodfellow_linalg`.
