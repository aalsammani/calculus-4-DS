# 4.2 · Power Series and Taylor Series

When a calculator reports $\sin(1) = 0.8414709848$, what did it actually compute? Not a triangle, not a unit circle — a *polynomial*. This section builds the bridge from series of numbers to series of functions: **power series**, sums of the form $\sum c_k x^k$, and the **Taylor series** that represent familiar functions in that form. The linearization of Section 2.4 was the first rung — matching a function's value and slope with a line; matching ever more derivatives climbs to quadratic, cubic, and ultimately infinite-degree approximations, with Taylor's theorem quantifying the error at every rung. The result is both a computational engine (how machines evaluate $e^x$, $\sin$, $\ln$) and a theoretical lens (why those functions are what they are).

## 4.2.1 Power series and where they converge
...
Within its radius, a power series behaves like the "infinite polynomial" it resembles: it may be **differentiated and integrated term by term**, and the resulting series have the *same radius*. This innocuous-sounding fact is a factory for new representations, as §4.2.3 will exploit.

## 4.2.2 Taylor series: which power series is a given function?
...
and its partial sum of degree $n$ is the **Taylor polynomial** $T_n(x)$ — the unique degree-$n$ polynomial matching $f$'s value and first $n$ derivatives at $a$. $T_1$ is exactly Section 2.4's linearization.
```
...
with $R = 1$ by the ratio test. At the endpoint $x = 1$ it converges (alternating series) to $\ln 2$ — Section 4.1's alternating harmonic series, now unmasked; at $x = -1$ it is the (divergent) harmonic series: endpoints genuinely differ.
```
...
Taylor polynomials of $\sin x$ at $0$. $T_1(x) = x$ is the small-angle approximation of Section 2.4; each added pair of terms extends the region of agreement outward. The polynomials eventually peel off and dive (polynomials must), but on any fixed interval, enough terms pin the curve exactly.
```

## 4.2.3 New series from old
...
an explicit, rapidly convergent formula for the "impossible" integral of Sections 3.2 and 3.5 — series succeed exactly where antiderivatives fail, which is much of why they matter.

**Differentiation.** Differentiating $\frac{1}{1-x} = \sum x^k$ gives $\frac{1}{(1-x)^2} = \sum_{k\ge1}k x^{k-1}$ (Section 4.1's Exercise 19, now fully licensed), and differentiating the $\sin$ series yields precisely the $\cos$ series — the identity $\sin' = \cos$ holding coefficient by coefficient.
...
**(b)** $\displaystyle\int_0^{1}e^{-t^2}dt$ to six digits: sum the integrated series at $x=1$: $1 - \frac13 + \frac{1}{10} - \frac{1}{42} + \frac{1}{216} - \cdots = 0.746824$, matching Section 3.5's `quad` to all shown digits after seven terms — alternating, so the first omitted term bounds the error.
```

## 4.2.4 Taylor's theorem: the error, exactly

How far can $T_n$ be trusted? The answer generalizes Section 2.4's linearization bound rung by rung:
...
The remainder is *the next term, with its derivative evaluated at an unknown intermediate point* — easy to remember, easy to bound. The case $n = 1$ is Section 2.4's $\frac M2(x-a)^2$ verbatim. And the factorial in the denominator is the engine of practical convergence: for $\sin x$ (all derivatives bounded by $1$),
...
— but the $x^8$ coefficient of sine's series is zero, so $T_7 = T_8$ and the sharper bound is $\frac{1}{9!} = 2.76\times10^{-6}$. The actual error, computed in §4.2.5, is $2.73\times10^{-6}$: the bound is honest almost to the digit. Four terms of a series certify five decimal places — this economy, guaranteed in advance by {prf:ref}`thm-taylor-remainder`, is precisely how numerical libraries decide how many terms to hard-code.
```
...
# --- Example 4.2.5(a): the limit, by series and by SymPy directly ---
print(sp.limit((sp.sin(x) - x)/x**3, x, 0))    # -1/6

# --- Example 4.2.6: measured error vs the remainder bound ---
import numpy as np
...
The last pair of numbers is the section in miniature: theory promised at most $2.76\times10^{-6}$, reality delivered $2.73\times10^{-6}$. Now watch convergence happen across a whole interval, and measure its speed at a point:
...
8. Write $T_3(x)$ for $f(x) = \sqrt{x}$ centered at $a = 4$, and use it to estimate $\sqrt{4.2}$, comparing with Section 2.4's linear estimate $2.05$ and the true $2.049390$.
