...
Taylor series approximate a function by polynomials, matching it perfectly at one point and trusting it outward. This section changes the basis and the philosophy: **Fourier series** approximate a *periodic* function by sines and cosines, matching it *on average across the whole period*. Where Taylor answers "what does $f$ look like near $a$?", Fourier answers "what frequencies is $f$ made of?" — the natural question for signals, seasonal data, sound, and vibration. The mathematics needed is exactly what Chapters 2–3 built: definite integrals, integration by parts, and the trigonometric identities of Section 1.2. This section treats the classical period-$2\pi$ theory; it is the optional capstone of Chapter 4 and the historical gateway to signal processing.

## 4.3.1 The idea: functions as combinations of waves
...
## 4.3.2 Orthogonality: the coefficient extractor

The harmonics obey a striking system of integral relations over one period (proved by the product-to-sum identities, or by the integration-by-parts computation you did in Section 3.4, Exercise 13):
...
## 4.3.3 Three classical expansions
...
Odd again, so sines only; integration by parts (Section 3.4, $u = x$, $dv = \sin kx\,dx$):
...
Even function: cosines only. The average is $a_0/2$ with $a_0 = \frac{2}{\pi}\int_0^\pi x^2dx = \frac{2\pi^2}{3}$. For $k \ge 1$, two rounds of parts (as in {prf:ref}`ex-parts-twice` of Section 3.4) give
...
The **Basel problem** — which defeated the Bernoullis and made Euler's name — falls out of one Fourier expansion evaluated at one point. Section 4.1 could only report that this $p$-series converges; now you possess its value, $1.644934\ldots$
```

## 4.3.4 What convergence means here
...
**Radian amnesia, final warning.** All of this section's integrals live in radians; the period is $2\pi$, not $360$.
```

## 4.3.6 Now do it in Python
...
Note the Basel check's gap in the sixth decimal: after *two hundred thousand* terms the tail $\sum_{k>N} k^{-2} \approx \frac1N$ still contributes $5\times10^{-6}$. The exact value from {prf:ref}`ex-fourier-parabola` beats brute-force summation decisively — analysis giving what computation only crawls toward, the same lesson Section 4.1's harmonic series taught in reverse.
...
:class: seealso
Orthogonality did all the work in this section, and it is a *linear-algebra* phenomenon: the harmonics behave like perpendicular axes, and Fourier coefficients are coordinates — projections onto those axes. Chapter 5 builds that language properly (dot products, orthogonal bases, projections), after which the coefficient formulas of {prf:ref}`def-fourier-coefficients` will re-read as one more instance of a universal recipe: coefficient = inner product with a unit basis vector.
```
...
8. Using the sawtooth series at $x = \frac\pi2$, derive the Leibniz formula $\frac\pi4 = 1 - \frac13 + \frac15 - \cdots$, and estimate how many terms it needs for 3-decimal accuracy via Section 4.1's alternating-series bound.
...
Decomposing a function into known pieces before integrating is as valuable here as it was in Section 3.2.
````
...
11. The square wave's partial sum with harmonics through $k = N$ has its first peak near $x = \frac{\pi}{N+1}$. Numerically locate the peak for $N = 25, 51, 101$ and confirm the overshoot height approaches the Gibbs value $\frac{2}{\pi}\int_0^\pi\frac{\sin t}{t}dt = 1.17898\ldots$ — the $\operatorname{Si}$ function of Section 3.1's challenge, resurfacing.
...
15. The orthogonality relations were called "perpendicularity" for functions. Propose the analogy explicitly: what plays the role of vectors, of the dot product, and of coordinates? *(Chapter 5 will grade your answer.)*
