# 6.3 · Gradients and Directional Derivatives

Partial derivatives answered "how steep is the terrain walking due east? due north?" — two directions out of infinitely many. This section answers all the rest at once, with a single vector: the **gradient** $\nabla f$, whose dot product with any unit direction prices the slope that way. Out of that one formula falls the section's headline geometry — the gradient points *straight uphill*, perpendicular to the contours, with magnitude the steepest available rate — and out of the geometry falls the most consequential algorithm in modern computing: to minimize a function, walk against its gradient. This is the section where the book's calculus and linear algebra finally run as one machine.

## 6.3.1 The gradient vector
...
## 6.3.2 Directional derivatives

Fix a point $\vb p$ and a **unit** direction $\vb u$. Walking from $\vb p$ along $\vb u$ traces the line $\vb r(t) = \vb p + t\vb u$ (Section 6.1), and the function's value along the walk is $g(t) = f(\vb r(t))$. Its rate at the start is *the slope of $f$ in the direction $\vb u$*:
...
Section 6.2's chain rule evaluates it instantly: with $x' = u_1$ and $y' = u_2$ along the line,
...
One vector, computed once, now answers every direction by dotting — and Section 5.2's angle formula reads the answer's structure out loud. Since $\nabla f\cdot\vb u = \|\nabla f\|\cos\theta$ ($\vb u$ being unit), where $\theta$ is the angle between the chosen direction and the gradient:
...
The perpendicularity of gradients and level sets is a workhorse in its own right: in three variables, $\nabla F$ at a point of the level *surface* $F(x,y,z) = c$ is that surface's **normal vector**, from which tangent planes to arbitrary surfaces follow by Section 5.2's Exercise 10 recipe (Exercise 10 below).

## 6.3.3 Critical points and the second-derivative test
...
As in Section 2.2, the condition is necessary, not sufficient: bowls (minima), domes (maxima), and — new in two dimensions — *saddles* all zero the gradient. The classifier is the second-order information, and Section 6.2's symmetric Hessian $\mathit H = \begin{bmatrix}f_{xx} & f_{xy}\\ f_{xy} & f_{yy}\end{bmatrix}$ is exactly built for the job: near a critical point, $f \approx f(\vb p) + \tfrac12\vb h^{\mathsf T}\mathit H\vb h$ (Section 6.2, Exercise 24), and the quadratic's shape is governed by $\mathit H$'s **eigenvalues** (real, with orthogonal eigenvectors, by the spectral theorem — the promised payoff of Section 5.6):
...
## 6.3.6 Now do it in Python

```python
import numpy as np
import sympy as sp

# --- Example 6.3.5 symbolically ---
x, y = sp.symbols('x y')
f = x**2*y
...
Try $\eta = 0.45$ (the $y$-coordinate oscillates, factor $1 - 4\eta = -0.8$), then $\eta = 0.55$ (divergence: $|1-4\eta| > 1$) — the theory of §6.3.4, watched live. And verify any symbolic gradient with Section 6.2's `grad_numeric`: agreement between the analytic and finite-difference gradients is the standard test (called a *gradient check*) for backpropagation code.
...
:class: tip
This section is the mathematical core of model training. The loss $L(\vb w)$ is a function of millions of parameters; backpropagation is the chain rule computing $\nabla L$; the optimizer steps $\vb w \leftarrow \vb w - \eta\nabla L$ (stochastic gradient descent estimates $\nabla L$ from mini-batches; momentum and Adam reshape the step); the learning rate is $\eta$; loss-landscape plots are contour maps; and convergence speed is the Hessian eigenvalue story of §6.3.4 — with feature standardization, batch normalization, and preconditioners all in the business of taming $\kappa = \lambda_{\max}/\lambda_{\min}$. A data scientist who owns {prf:ref}`thm-gradient-geometry` and §6.3.4 owns the *why* behind a large fraction of the field's daily vocabulary.
```

```{admonition} Looking Ahead
:class: seealso
Differentiation in several variables is now complete: partials, gradients, directions, optimization. Integration's turn — Section 6.4 sums $f(x,y)$ over two-dimensional regions (volumes, averages, probabilities), and Section 6.5 changes coordinates under the integral, with Section 5.5's determinant pricing the area distortion.
```
...
6. Find the equation of the tangent plane to the *level surface* $x^2 + y^2 + z^2 = 14$ at $(1, 2, 3)$, using $\nabla F$ as the normal (Section 5.2's plane recipe). Check that the result matches the geometric fact that a sphere's tangent plane is perpendicular to its radius.
7. Find and classify all critical points: (a) $f = x^2 + y^2 - 4x + 6y$; (b) $f = xy(3 - x - y)$; (c) $f = x^4 + y^4 - 4xy$.
8. For $f = x^2 + 2y^2$, verify at three points of your choosing that $\nabla f$ is perpendicular to the level ellipse through the point (compute the contour's tangent by implicit differentiation, Section 2.3, and dot).
...
11. Do the full §6.3.4 analysis for $f = 5x^2 + y^2$: Hessian eigenvalues, the exact stability threshold for $\eta$, the per-step factors at $\eta = 0.15$, and the number of steps for the *slow* mode's error to fall below $10^{-6}$ of its start (Section 1.2 logarithms).
12. **Constrained hint of things to come.** Maximize $f(x, y) = xy$ on the circle $x^2 + y^2 = 2$ by parametrizing the circle ($x = \sqrt2\cos t$, $y = \sqrt2\sin t$), reducing to one variable, and optimizing with Section 2.5. Then observe: at your maximizer, $\nabla f$ is parallel to the constraint's gradient $\nabla(x^2+y^2)$ — the pattern behind Lagrange multipliers, which handle such problems without parametrizing.
