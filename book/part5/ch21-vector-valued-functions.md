...
Chapter 6 reunites the two halves of the book. Chapters 2–4 did calculus on functions with one input and one output; Chapter 5 built the algebra of vectors. Now they merge — beginning with the simplest hybrid, a function whose *input* is a single number (time, usually) and whose *output* is a vector: a moving point. Everything from Chapter 2 transfers with almost embarrassing ease — derivatives componentwise, the derivative *is* velocity, integrals recover position — and the payoff is the mathematics of trajectories: orbits, projectiles, robot arms, and (in data terms) any quantity that traces a path through a feature space over time.

## 6.1.1 Curves as functions
...
with **component functions** $x(t), y(t), \ldots$ — ordinary functions of Chapter 2. As $t$ runs, the tip of $\vb r(t)$ traces a **curve**; the function is the curve *plus a schedule* for traversing it (a **parametrization**).
```

The distinction between curve and parametrization matters: $\vb r(t) = \langle\cos t, \sin t\rangle$ and $\vb s(t) = \langle\cos 2t, \sin 2t\rangle$ trace the same unit circle, the second at twice the speed. A curve is a road; a parametrization is a particular drive along it. Familiar examples: the **line** through point $\vb p$ with direction $\vb d$ is $\vb r(t) = \vb p + t\,\vb d$ (Section 5.1's Exercise 10, now official — with $t \in [0,1]$ and $\vb d = \vb q - \vb p$ it traces the segment from $\vb p$ to $\vb q$); the circle of radius $a$ is $\langle a\cos t, a\sin t\rangle$; and the graph of any old-fashioned function $f$ is the special case $\langle t, f(t)\rangle$ — Chapter 2 was secretly studying curves that never doubled back.
...
The limit's numerator $\vb r(t+h) - \vb r(t)$ is a chord vector (Section 5.1's tip-minus-tail); dividing by $h$ and shrinking makes the chord swing into tangency — Section 2.1's secant-to-tangent story, replayed with arrows. Note the vocabulary split that one-variable calculus never needed: **velocity** is a vector (direction and rate), **speed** is its scalar magnitude.
...
Three classical facts drop out. The speed is constant ($1$ radian of arc per unit time — the arc-length meaning of radians from Section 1.2). The velocity is perpendicular to the position — $\vb r\cdot\vb v = -\cos t\sin t + \sin t\cos t = 0$ — tangent to the circle, as {numref}`fig-parametric-motion` shows (this is Section 5.3's Exercise 23, now in its natural home). And the acceleration points *opposite the position*: straight at the center. Uniform circular motion accelerates centripetally not by slowing down but by *turning* — a vector can change with constant magnitude, something one-variable calculus could not even express.
```
...
(orders preserved in the cross case — Section 5.2's anticommutativity demands it). One elegant dividend: if $\|\vb r(t)\|$ is constant, then $\vb r\cdot\vb r$ is constant, so differentiating gives $2\,\vb r\cdot\vb r' = 0$ — *motion on a sphere is always perpendicular to the position vector*, the general truth behind {prf:ref}`ex-circular-motion`.
...
The formula is Section 3.1 in disguise — distance is the integral of speed — and its inner Pythagorean expression is the length of the tiny displacement $\vb r'(t)\,dt$ in each instant: chop the curve into near-straight steps, add their lengths, take the limit. Riemann's recipe, aimed at a curve.
...
— by the trig substitution $2t = \tan\theta$ of Section 3.4 (or a table). The pattern is typical: arc-length integrands carry square roots, most have no elementary antiderivative (the ellipse's famously doesn't), and numerical integration (Section 3.5) is the standard tool. The *formula* is simple; the *integrals* are where hand methods retire.
```

## 6.1.3 Common mistakes
...
**Degrees again.** Parametrizations with $\cos t$, $\sin t$ presume radians; the derivative rules require it (Section 2.3's warning, permanently in force).
```

## 6.1.4 Now do it in Python
...
**Interpretation.** Reading trajectories through velocity and acceleration vectors is a transferable skill: in data settings, $\vb r(t)$ might be a customer's position in feature space over months, an optimizer's parameter vector over training steps, or a sensor's state over time; "which direction is it heading, and how fast?" is $\vb r'$, and "is the movement turning or settling?" is $\vb r''$. Gradient-descent trajectories in Section 6.3 will be exactly such curves.
...
:class: seealso
This section varied *time* and watched a vector output. The next varies *space*: functions $f(x, y)$ with a vector input and scalar output — surfaces, contour maps, and the question "how does $f$ change as I move in this direction?", whose answer (Section 6.3) multiplies this section's velocity vectors by the next section's partial derivatives via a chain rule.
```
...
9. For projectile motion at fixed $v_0$, show the range $R(\alpha) = \frac{v_0^2\sin2\alpha}{g}$ and use Section 2.2's optimization to prove $\alpha = 45°$ maximizes it. What does {prf:ref}`ex-projectile`'s $50°$ range lose relative to the optimum?
10. Prove the constant-magnitude theorem from §6.1.1 formally — if $\|\vb r(t)\| = c$ for all $t$ then $\vb r\cdot\vb r' \equiv 0$ — and apply it: why must the Earth's orbital velocity be perpendicular to its position vector *only if* the orbit is circular?
11. The cycloid is $\vb r(t) = \langle t - \sin t,\ 1 - \cos t\rangle$. Compute $\vb v(t)$, show the speed is $\sqrt{2 - 2\cos t} = 2\left|\sin\frac t2\right|$ (Section 1.2's half-angle identity), and find the arc length of one arch ($0 \le t \le 2\pi$). *(Answer: 8 — an arch is exactly 8 radii long, a classical gem.)*
...
13. Explain the difference between $\vb r'(t_0)$, the tangent *line* at $t_0$, and the tangent line's direction being non-unique (any multiple works) — connecting to eigen-directions' scale freedom from Section 5.6.
...
17. Write `arc_length(r, a, b)` taking a callable `r(t)` returning an array, estimating length by summing `np.linalg.norm(np.diff(pts, axis=0), axis=1)` over a fine grid. Test against the helix's exact $2\sqrt2\pi$ and report the error at 100, 1000, and 10000 grid points; what convergence order do you observe, and which Section 3.5 rule does this method secretly resemble?
...
22. *(§3.3–3.4)* The curve $\langle t, \cosh t\rangle$ (the hanging-chain catenary, with $\cosh t = \frac{e^t + e^{-t}}{2}$) has speed $\sqrt{1 + \sinh^2 t} = \cosh t$. Find the arc length over $[0, 1]$ exactly, and note the rare pleasure: an arc-length integral with a clean antiderivative.
23. *(§5.6)* The linear system $\vb x_{k+1} = \mathit A\vb x_k$ of Section 5.6 is a *discrete-time* trajectory. For $\mathit A = \begin{bmatrix}0.96 & -0.20\\ 0.20 & 0.96\end{bmatrix}$ (a slight rotation-with-shrink: eigenvalues $0.96 \pm 0.20i$, magnitude $\approx 0.98$), iterate 200 steps from $\langle 1, 0\rangle$ and plot the points. Describe the curve and connect its inward spiral to the eigenvalue magnitude.
