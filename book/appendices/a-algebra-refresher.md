...
The discriminant $b^2 - 4ac$ counts real roots (two if positive, one if zero, none if negative) — the fact behind Section 5.5's characteristic polynomials having two, one, or no real eigenvalues. Completing the square, $x^2 + bx = \left(x + \frac b2\right)^2 - \frac{b^2}{4}$, is used in Section 6.3 (classifying quadratics) and Section 6.5 (Gaussian integrals).
...
For $a, b > 0$ (Section 1.2 derives these from exponent laws):
...
Multiplying an inequality by a negative number reverses it. $|x| < a \iff -a < x < a$; $|x| > a \iff x > a$ or $x < -a$. The triangle inequality $|a + b| \le |a| + |b|$ reappears for vectors in Section 5.1.

## A.6 Lines and slopes

Through $(x_0, y_0)$ with slope $m$: $\ y = y_0 + m(x - x_0)$ — the *point–slope form*, deliberately memorized in this arrangement because Section 2.4's linearization $L(x) = f(a) + f'(a)(x - a)$ **is** this form. Parallel lines share $m$; perpendicular lines have slopes multiplying to $-1$.
...
The index is a bound variable ($\sum_k a_k = \sum_j a_j$); constants factor out; sums split over addition. Section 4.1 builds on this notation heavily; Section 3.1's Riemann sums use it from the start.
