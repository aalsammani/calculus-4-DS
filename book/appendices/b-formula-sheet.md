...
One page per part; the formulas this book expects at your fingertips, with chapter references.

## Differential calculus (§2.1–§2.5)
...
**Optimization.** Interior extrema only at critical points ($f' = 0$ or undefined). First-derivative test: sign of $f'$ flips $+\to-$ (max), $-\to+$ (min), no flip (neither). Second-derivative test: $f''(c)>0$ min, $f''(c)<0$ max, $f''(c)=0$ inconclusive. Closed interval: compare $f$ at critical points and endpoints.

## Integral calculus (§3.1–§3.5)
